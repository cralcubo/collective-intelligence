...
private fun Entity.mergeTo(e2: Entity) =
    Entity(UUID.randomUUID().toString(), this.features.intersect(e2.features))
...
private fun createItems(genres: Set<String>, tags:Set<String>): Set<Feature> {
    val gi = genres.map { Feature(it.lowercase(), 1.0) }.toSet()
    val gt = tags.map { Feature(it.lowercase(), 1.0) }.toSet()
    return gi + gt
}
...
private fun runTagByEntity(entityId: String) {
    ...
    val entities = tags.mapNotNull {
        movies.find { m -> m.movieId == it.movieId }?.let { m ->
            Entity(m.title, it.tags.map { t -> Feature(t.lowercase(), 1.0) }.toSet() )
        }
    }
...
private fun runTag() {
    ...
    val entities = tags.mapNotNull {
        movies.find { m -> m.movieId == it.movieId }?.let { m ->
            Entity(m.title, it.tags.map { t -> Feature(t, 1.0) }.toSet() )
        }
    }
    runAll(entities)
}
...
private fun run() {
    ...
    val entities = movies.map {
        Entity(it.title, it.genres.map { g -> Feature(g, 1.0) }.toSet())
    }
    runAll(entities)
}
...
private fun Cluster.print() {
    val itemsStr : (Entity)-> String = { it.features.joinToString { i -> i.id } }
    when(this) {
