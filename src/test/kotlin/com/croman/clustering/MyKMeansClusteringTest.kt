...
class MyKMeansClusteringTest {
    ...
    fun unbalancedClusters() {
        ...
        kMeans.map { it.centroid } shouldContainExactlyInAnyOrder listOf(Pair(9, 5),Pair(2, 3))
        kMeans.find { it.centroid == Pair(9, 5) }!!.dataPoints shouldContainExactlyInAnyOrder listOf(Pair(9, 6), Pair(9, 4), Pair(8, 5), Pair(10,5))
        kMeans.find { it.centroid == Pair(2, 3) }!!.dataPoints shouldContainExactlyInAnyOrder listOf(Pair(2, 2), Pair(2, 4))
    }
